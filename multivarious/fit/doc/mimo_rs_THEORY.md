...
mimo_rs approximates the input-output relationship as:
...
For multiple outputs, mimo_rs fits **separate models** for each output:
...
(-1/2)(-1/2)### 3.3 Low-Order Hermite Functions
...
- option to log-transform, scale, standardize or decorrelate the data
...
- Fit full model using least squares with L1 regularization
...
The transformations are stored as:  $a^{(-1/2)}$
...
(-1/2)(-1/2)**Inverse transformation** (for predictions):
...
```
|Zᵢⱼ| the Chauventet criterion threshold 
```

Here, a simple approximiation to Chauvent's criterion is 0.8 + 0.4 log( number of data points )
...
### 4.3 Least Squares Fitting with L1 Regularization
...
Solution (of the KKT system via SVD for numerical stability):
...
### 5.2 Adjusted R-Squared (R²_adj)

Penalizes model complexity:

```
min ||B·c - Y|_2 + a ||c||_1
 cmin ||B·c - Y|_2 + a ||c||_1
 cR²_adj = ((m-1)·R² - nTerm) / (m - nTerm)
```
...
*Based on a translation and enhancement of the mimoSHORSA implementation*
