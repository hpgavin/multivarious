"""
chrip.py - Generate frequency-swept signals for structural testing
...
Original: chrip.m by Henri Gavin, Duke University, 1999
"""
...
def chrip(ao, af, fo, ff, t, p=2, n=1, phi=90, fig_no=0, units='m'):
    ...
    Creates a frequency-swept signal (chrip) with exponentially varying amplitude,
    ...
    veloc : ndarray
        Velocity time history (fundamental chrip signal)
    displ : ndarray
    ...
    >>> accel, veloc, displ = chrip(1.0, 0.2, 0.2, 20, t, p=2)
    >>> # Use 'displ' for displacement control
    >>> # Use 'accel' for force estimation
    >>> 
    >>> # Example 3: Linear frequency sweep
    >>> accel, veloc, displ = chrip(1.0, 0.5, 1, 10, t, p=1)
    >>> 
    >>> # Example 4: Start at maximum (no phase control)
    >>> accel, veloc, displ = chrip(1.0, 0.5, 1, 10, t, phi=0)
    
    See Also
    --------
    scipy.signal.chirp     : Standard chirp (constant amplitude, single output)
    multivarious.dsp.taper : Tapering function used internally
    multivarious.dsp.cdiff : Central difference differentiation used internally
...
if __name__ == '__main__':
    """
    Test chrip function with various configurations
    """
    
    print("\n" + "="*70)
    print("Testing chrip.py")
    print("="*70)
    ...
    # Test 5: Comparison with scipy.signal.chrip
    ...
    # chrip (constant amplitude for fair comparison)
    accel_ours, veloc_ours, displ_ours = chrip(
    ...
    ax1 = plt.subplot(3, 1, 1)
    ax1.plot(t5, veloc_ours, 'b-', linewidth=1.5, label='multivarious chrip()', alpha=0.8)
    ax1.plot(t5, scipy_signal, 'r--', linewidth=1.5, label='scipy.signal.chirp()', alpha=0.6)
    ax1.set_ylabel('Signal', fontsize=11)
    ax1.set_title('Comparison: multivarious.dsp.chrip() vs scipy.signal.chirp()', 
                 fontsize=12, fontweight='bold')
    ...
    ax2 = plt.subplot(3, 1, 2)
    ax2.plot(t5, displ_ours, 'g-', linewidth=1.5, label='multivarious: displacement')
    ...
    ax3 = plt.subplot(3, 1, 3)
    ax3.plot(t5, accel_ours, 'm-', linewidth=1.5, label='multivarious: acceleration')
    ax3.set_xlabel('Time (s)', fontsize=11)
