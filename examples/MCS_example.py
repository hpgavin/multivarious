...
sdv1 = 2        # std dev of normal
#  X2 lognormal  
med2 = 2        # median of lognormal
cov2 = 0.3      # coefficient of variation of lognormal
#  X3 Rayleigh
...
X1 = normal.rnd(mean1, sdv1, N)
X2 = lognormal.rnd(med2, cov2, N)
...
sort_X2 = np.sort(X2)
sort_X3 = np.sort(X3)
CDF = np.arange(1, N + 1) / (N + 1)  # empirical CDF of all quantities

nBins = int(np.floor(N / 50))

plt.ion()

format_plot(line_width = 1, font_size = 14, marker_size = 5)
ppl = np.array([48, 10, 84]) / 256 # Purple color to match histogram bars
...
plt.subplot(2, 3, 5)
plt.step(sort_X2, CDF, where='post', color=ppl, linewidth=2)
plt.plot(sort_X2, lognormal.cdf(sort_X2, med2, cov2), 'k-', linewidth=0.5)
...
plt.subplot(2, 3, 6)
plt.step(sort_X3, CDF, where='post', color=ppl, linewidth=2)
plt.plot(sort_X3, rayleigh.cdf(sort_X3, mean3), 'k-', linewidth=0.5)
